...
public class RegisterActivity extends AppCompatActivity {
    ...
    private void CreateAccount() {
        ...
        String regexStr = "^[+]?[0-9]{10,010}$";

        if (TextUtils.isEmpty(name)){
            Toast.makeText(this, "Please Write Your Full Names.....", Toast.LENGTH_LONG).show();
        }
        else if (TextUtils.isEmpty(phone)){
            Toast.makeText(this, "Please Write Phone Number.....", Toast.LENGTH_LONG).show();
        }

        String number=InputPhoneNumber.getText().toString();

        if(InputPhoneNumber.getText().toString().length()<10 || number.length()>10 || number.matches(regexStr)==false  ) {
            Toast.makeText(RegisterActivity.this,"Please enter "+"\n"+" valid phone number",Toast.LENGTH_LONG).show();
            // am_checked=0;
        }
        ...
        RootRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (!(dataSnapshot.child("Users").child(phone).exists())){
                    ...
                    loadingBar.dismiss();
                    Toast.makeText(RegisterActivity.this, "Please try again using different phone number...", Toast.LENGTH_LONG).show();
